...
namespace BookStore.Controllers.MVC.ItemInvoices
{
    ...
    public class SellInvoicesController : Controller
    {
        ...
        private readonly SellInvoiceManager sellInvoiceManager;

        //View page addresses
        private const string INDEX = "~/Views/ItemInvoices/SellInvoices/Index.cshtml";
        private const string CREATE = "~/Views/ItemInvoices/SellInvoices/Create.cshtml";
        private const string EDIT = "~/Views/ItemInvoices/SellInvoices/Edit.cshtml";
        private const string INVOICE = "~/Views/ItemInvoices/SellInvoices/Invoice.cshtml";

        public SellInvoicesController()
        {
            bookStoreUnitOfWork = new BookStoreUnitOfWork();
            sellInvoiceManager = new SellInvoiceManager(bookStoreUnitOfWork);
        }
        ...
        public async Task<ActionResult> Index(ItemInvoiceFilter filter = null)
        {
            ...
            return View(INDEX,
                (await sellInvoiceManager.GetAllNoTrackingAsync(filter))
        ...
        public async Task<ActionResult> Invoice(long id)
        {
            ...
            model = await bookStoreUnitOfWork.SellInvoices
        ...
        public async Task<ActionResult> Create(CreateEditItemInvoiceViewModel model,string returnUrl)
        {
            try
            {
                ...
                {
                    bookStoreUnitOfWork.SellInvoices.Add(model,User.Identity.GetUserId());
        ...
        public async Task<ActionResult> Edit(long id)
        {
            ...
            model = await bookStoreUnitOfWork.SellInvoices.GetNoTrackingWithIncludesAsync(id, includeItems: true);
        ...
        public async Task<ActionResult> Edit(long id, CreateEditItemInvoiceViewModel model, string returnUrl)
        {
            try
            {
                if (ModelState.IsValid && model.Items.Count > 0)
                {
                    await sellInvoiceManager.Edit(model, User.Identity.GetUserId());

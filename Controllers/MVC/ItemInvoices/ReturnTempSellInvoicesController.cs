...
namespace BookStore.Controllers.MVC.ItemInvoices
{
    ...
    public class ReturnTempSellInvoicesController : Controller
    {
        private readonly BookStoreUnitOfWork bookStoreUnitOfWork;
        private readonly ReturnTempSellInvoiceManager returnTempSellInvoiceManager;

        //View page addresses
        private const string INDEX = "~/Views/ItemInvoices/ReturnTempSellInvoices/Index.cshtml";
        private const string CREATE = "~/Views/ItemInvoices/ReturnTempSellInvoices/Create.cshtml";
        private const string EDIT = "~/Views/ItemInvoices/ReturnTempSellInvoices/Edit.cshtml";
        private const string INVOICE = "~/Views/ItemInvoices/ReturnTempSellInvoices/Invoice.cshtml";

        public ReturnTempSellInvoicesController()
        {
            bookStoreUnitOfWork = new BookStoreUnitOfWork();
            returnTempSellInvoiceManager = new ReturnTempSellInvoiceManager(bookStoreUnitOfWork);
        }
        ...
        public async Task<ActionResult> Index(ItemInvoiceFilter filter = null)
        {
            ...
            return View(INDEX,
                (await returnTempSellInvoiceManager.GetAllNoTrackingAsync(filter))
        ...
        public async Task<ActionResult> Invoice(long id)
        {
            ...
            model = await bookStoreUnitOfWork.ReturnTempSellInvoices
        ...
        public async Task<ActionResult> Create(CreateEditItemInvoiceViewModel model, string returnUrl)
        {
            try
            {
                ...
                {
                    bookStoreUnitOfWork.ReturnTempSellInvoices.Add(model, User.Identity.GetUserId());
        ...
        public async Task<ActionResult> Edit(long id)
        {
            ...
            model = await bookStoreUnitOfWork.ReturnTempSellInvoices.GetNoTrackingWithIncludesAsync(id, includeItems: true);
        ...
        public async Task<ActionResult> Edit(long id, CreateEditItemInvoiceViewModel model, string returnUrl)
        {
            try
            {
                if (ModelState.IsValid && model.Items.Count > 0)
                {
                    await returnTempSellInvoiceManager.Edit(model, User.Identity.GetUserId());

...
namespace BookStore.Controllers.MVC.ItemInvoices
{
    ...
    public class ReturnSellInvoicesController : Controller
    {
        private readonly BookStoreUnitOfWork bookStoreUnitOfWork;
        private readonly ReturnSellInvoiceManager returnSellInvoiceManager;

        //View page addresses
        private const string INDEX = "~/Views/ItemInvoices/ReturnSellInvoices/Index.cshtml";
        private const string CREATE = "~/Views/ItemInvoices/ReturnSellInvoices/Create.cshtml";
        private const string EDIT = "~/Views/ItemInvoices/ReturnSellInvoices/Edit.cshtml";
        private const string INVOICE = "~/Views/ItemInvoices/ReturnSellInvoices/Invoice.cshtml";

        public ReturnSellInvoicesController()
        {
            bookStoreUnitOfWork = new BookStoreUnitOfWork();
            returnSellInvoiceManager = new ReturnSellInvoiceManager(bookStoreUnitOfWork);
        }
        ...
        public async Task<ActionResult> Index(ItemInvoiceFilter filter = null)
        {
            ...
            return View(INDEX,
                (await returnSellInvoiceManager.GetAllNoTrackingAsync(filter))
        ...
        public async Task<ActionResult> Invoice(long id)
        {
            ...
            model = await bookStoreUnitOfWork.ReturnSellInvoices
        ...
        public async Task<ActionResult> Create(CreateEditItemInvoiceViewModel model, string returnUrl)
        {
            try
            {
                ...
                {
                    bookStoreUnitOfWork.ReturnSellInvoices.Add(model, User.Identity.GetUserId());
        ...
        public async Task<ActionResult> Edit(long id)
        {
            ...
            model = await bookStoreUnitOfWork.ReturnSellInvoices.GetNoTrackingWithIncludesAsync(id, includeItems: true);
        ...
        public async Task<ActionResult> Edit(long id, CreateEditItemInvoiceViewModel model, string returnUrl)
        {
            try
            {
                if (ModelState.IsValid && model.Items.Count > 0)
                {
                    await returnSellInvoiceManager.Edit(model, User.Identity.GetUserId());

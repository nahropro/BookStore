...
namespace BookStore.Controllers.MVC.ItemInvoices
{
    ...
    public class ReturnBuyInvoicesController : Controller
    {
        private readonly BookStoreUnitOfWork bookStoreUnitOfWork;
        private readonly ReturnBuyInvoiceManager returnBuyInvoiceManager;

        //View page addresses
        private const string INDEX = "~/Views/ItemInvoices/ReturnBuyInvoices/Index.cshtml";
        private const string CREATE = "~/Views/ItemInvoices/ReturnBuyInvoices/Create.cshtml";
        private const string EDIT = "~/Views/ItemInvoices/ReturnBuyInvoices/Edit.cshtml";
        private const string INVOICE = "~/Views/ItemInvoices/ReturnBuyInvoices/Invoice.cshtml";

        public ReturnBuyInvoicesController()
        {
            bookStoreUnitOfWork = new BookStoreUnitOfWork();
            returnBuyInvoiceManager = new ReturnBuyInvoiceManager(bookStoreUnitOfWork);
        }
        ...
        public async Task<ActionResult> Index(ItemInvoiceFilter filter = null)
        {
            ...
            return View(INDEX,
                (await returnBuyInvoiceManager.GetAllNoTrackingAsync(filter))
        ...
        public async Task<ActionResult> Invoice(long id)
        {
            ...
            model = await bookStoreUnitOfWork.ReturnBuyInvoices
        ...
        public async Task<ActionResult> Create(CreateEditItemInvoiceViewModel model, string returnUrl)
        {
            try
            {
                ...
                {
                    bookStoreUnitOfWork.ReturnBuyInvoices.Add(model, User.Identity.GetUserId());
        ...
        public async Task<ActionResult> Edit(long id)
        {
            ...
            model = await bookStoreUnitOfWork.ReturnBuyInvoices.GetNoTrackingWithIncludesAsync(id, includeItems: true);
        ...
        public async Task<ActionResult> Edit(long id, CreateEditItemInvoiceViewModel model, string returnUrl)
        {
            try
            {
                if (ModelState.IsValid && model.Items.Count > 0)
                {
                    await returnBuyInvoiceManager.Edit(model, User.Identity.GetUserId());

...
namespace BookStore.Controllers.MVC.ItemInvoices
{
    ...
    public class BuyInvoicesController : Controller
    {
        private readonly BookStoreUnitOfWork bookStoreUnitOfWork;
        private readonly BuyInvoiceManager buyInvoiceManager;

        //View page addresses
        private const string INDEX = "~/Views/ItemInvoices/BuyInvoices/Index.cshtml";
        private const string CREATE = "~/Views/ItemInvoices/BuyInvoices/Create.cshtml";
        private const string EDIT = "~/Views/ItemInvoices/BuyInvoices/Edit.cshtml";
        private const string INVOICE = "~/Views/ItemInvoices/BuyInvoices/Invoice.cshtml";

        public BuyInvoicesController()
        {
            bookStoreUnitOfWork = new BookStoreUnitOfWork();
            buyInvoiceManager = new BuyInvoiceManager(bookStoreUnitOfWork);
        }
        ...
        public async Task<ActionResult> Index(ItemInvoiceFilter filter = null)
        {
            ...
            return View(INDEX,
                (await buyInvoiceManager.GetAllNoTrackingAsync(filter))
        ...
        public async Task<ActionResult> Invoice(long id)
        {
            ...
            model = await bookStoreUnitOfWork.BuyInvoices
        ...
        public async Task<ActionResult> Create(CreateEditItemInvoiceViewModel model, string returnUrl)
        {
            try
            {
                ...
                {
                    bookStoreUnitOfWork.BuyInvoices.Add(model, User.Identity.GetUserId());
        ...
        public async Task<ActionResult> Edit(long id)
        {
            ...
            model = await bookStoreUnitOfWork.BuyInvoices.GetNoTrackingWithIncludesAsync(id, includeItems: true);
        ...
        public async Task<ActionResult> Edit(long id, CreateEditItemInvoiceViewModel model, string returnUrl)
        {
            try
            {
                if (ModelState.IsValid && model.Items.Count > 0)
                {
                    await buyInvoiceManager.Edit(model, User.Identity.GetUserId());

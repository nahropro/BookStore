...
{
    public class SpendReportsController : Controller
    {
        private readonly BookStoreUnitOfWork bookStoreUnitOfWork;
        private const string INDEX = "~/Views/SpendIncomeReports/SpendReports/Index.cshtml";

        public SpendReportsController()
        {
        ...
        public async Task<ActionResult> Index(SpendIncomeReportFilter filter)
        {
            ...
            ViewBag.TypeIds = (await bookStoreUnitOfWork.SpendTypes.GetAllNoTrackingAsync()).Select(i => new KeyValuePairViewModel<long, string>
            {
                Key = i.Id,
                Value = i.Name,
            }).ToList();

            return View(INDEX,await bookStoreUnitOfWork.SpendItems.GetSpendReportAsync(filter));
        }
    }
}
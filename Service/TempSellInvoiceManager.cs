...
{
    public class TempSellInvoiceManager
    {
        private readonly BookStoreUnitOfWork bookStoreUnitOfWork;

        public TempSellInvoiceManager(BookStoreUnitOfWork bookStoreUnitOfWork)
        {
        ...
        public TempSellInvoiceManager(BookStoreDbContext bookStoreDbContext = null)
        {
        ...
        public async Task<TempSellInvoice> Edit(CreateEditItemInvoiceViewModel model, string userId)
        {
            TempSellInvoice invoice;
            ...
            invoice = await bookStoreUnitOfWork.TempSellInvoices.GetWithIncludesAsync(model.Id, includeItems: true);

            if (invoice != null)
            {
                //Remove all items, then after add edited items
                bookStoreUnitOfWork.TempSellItems.RemoveRange(invoice.Items);
                ...
                invoice.Items = model.Items.Select(i => new TempSellItem
                {

...
namespace BookStore.Service
{
    public class BuyInvoiceManager
    {
        ...
        public BuyInvoiceManager(BookStoreUnitOfWork bookStoreUnitOfWork)
        {
        ...
        public BuyInvoiceManager(BookStoreDbContext bookStoreDbContext = null)
        {
        ...
        public async Task<BuyInvoice> Edit(CreateEditItemInvoiceViewModel model, string userId)
        {
            BuyInvoice invoice;
            ...
            invoice = await bookStoreUnitOfWork.BuyInvoices.GetWithIncludesAsync(model.Id, includeItems: true);

            if (invoice != null)
            {
                //Remove all items, then after add edited items
                bookStoreUnitOfWork.BuyItems.RemoveRange(invoice.Items);
                ...
                invoice.Items = model.Items.Select(i => new BuyItem
                {

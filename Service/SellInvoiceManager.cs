...
{
    public class SellInvoiceManager
    {
        private readonly BookStoreUnitOfWork bookStoreUnitOfWork;

        public SellInvoiceManager(BookStoreUnitOfWork bookStoreUnitOfWork)
        {
        ...
        public SellInvoiceManager(BookStoreDbContext bookStoreDbContext = null)
        {
        ...
        public async Task<SellInvoice> Edit(CreateEditItemInvoiceViewModel model,string userId)
        {
            SellInvoice invoice;
            ...
            invoice =await bookStoreUnitOfWork.SellInvoices.GetWithIncludesAsync(model.Id,includeItems:true);

            if (invoice!=null)
            {
                //Remove all items, then after add edited items
                bookStoreUnitOfWork.SellItems.RemoveRange(invoice.Items);
                ...
                invoice.Items = model.Items.Select(i => new SellItem
                {

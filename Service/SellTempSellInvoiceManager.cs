...
{
    public class SellTempSellInvoiceManager
    {
        private readonly BookStoreUnitOfWork bookStoreUnitOfWork;

        public SellTempSellInvoiceManager(BookStoreUnitOfWork bookStoreUnitOfWork)
        {
        ...
        public SellTempSellInvoiceManager(BookStoreDbContext bookStoreDbContext = null)
        {
            this.bookStoreUnitOfWork = new BookStoreUnitOfWork(bookStoreDbContext);
        }

        public async Task<List<SelectSellTempSellInvoiceViewModel>> GetAllNoTrackingAsync(ItemInvoiceFilter filter = null)
        {
            List<SelectSellTempSellInvoiceViewModel> result;
            List<SellTempSellInvoice> invoices;
            List<ApplicationUser> users;
            List<string> userIds = new List<string>();

            //Get all invoice with include nessesary objects and filtered if has any
            invoices = await bookStoreUnitOfWork.SellTempSellInvoices
            ...
            result = invoices.Select(i => new SelectSellTempSellInvoiceViewModel
            {
                Id = i.Id,
        ...
        public async Task<SellTempSellInvoice> Edit(CreateEditSellTempSellInvoiceViewModel model, string userId)
        {
            ...
            invoice = await bookStoreUnitOfWork.SellTempSellInvoices.GetWithIncludesAsync(model.Id, includeItems: true);

            if (invoice != null)
            {
                //Remove all items, then after add edited items
                bookStoreUnitOfWork.SellTempSellItems.RemoveRange(invoice.Items);
                ...
                invoice.Items = model.Items.Select(i => new SellTempSellItem
                {

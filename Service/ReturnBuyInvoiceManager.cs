...
{
    public class ReturnBuyInvoiceManager
    {
        private readonly BookStoreUnitOfWork bookStoreUnitOfWork;

        public ReturnBuyInvoiceManager(BookStoreUnitOfWork bookStoreUnitOfWork)
        {
        ...
        public ReturnBuyInvoiceManager(BookStoreDbContext bookStoreDbContext = null)
        {
        ...
        public async Task<List<SelectItemInvoiceViewModel>> GetAllNoTrackingAsync(ItemInvoiceFilter filter = null)
        {
            ...
            List<ReturnBuyInvoice> invoices;
            List<ApplicationUser> users;
            List<string> userIds = new List<string>();

            //Get all invoice with include nessesary objects and filtered if has any
            invoices = await bookStoreUnitOfWork.ReturnBuyInvoices
        ...
        public async Task<ReturnBuyInvoice> Edit(CreateEditItemInvoiceViewModel model, string userId)
        {
            ReturnBuyInvoice invoice;
            ...
            invoice = await bookStoreUnitOfWork.ReturnBuyInvoices.GetWithIncludesAsync(model.Id, includeItems: true);

            if (invoice != null)
            {
                //Remove all items, then after add edited items
                bookStoreUnitOfWork.ReturnBuyItems.RemoveRange(invoice.Items);
                ...
                invoice.Items = model.Items.Select(i => new ReturnBuyItem
                {

...
{
    public class ReturnSellInvoiceManager
    {
        private readonly BookStoreUnitOfWork bookStoreUnitOfWork;

        public ReturnSellInvoiceManager(BookStoreUnitOfWork bookStoreUnitOfWork)
        {
        ...
        public ReturnSellInvoiceManager(BookStoreDbContext bookStoreDbContext = null)
        {
        ...
        public async Task<List<SelectItemInvoiceViewModel>> GetAllNoTrackingAsync(ItemInvoiceFilter filter = null)
        {
            ...
            List<ReturnSellInvoice> invoices;
            List<ApplicationUser> users;
            List<string> userIds = new List<string>();

            //Get all invoice with include nessesary objects and filtered if has any
            invoices = await bookStoreUnitOfWork.ReturnSellInvoices
        ...
        public async Task<ReturnSellInvoice> Edit(CreateEditItemInvoiceViewModel model, string userId)
        {
            ReturnSellInvoice invoice;
            ...
            invoice = await bookStoreUnitOfWork.ReturnSellInvoices.GetWithIncludesAsync(model.Id, includeItems: true);

            if (invoice != null)
            {
                //Remove all items, then after add edited items
                bookStoreUnitOfWork.ReturnSellItems.RemoveRange(invoice.Items);
                ...
                invoice.Items = model.Items.Select(i => new ReturnSellItem
                {

...
function lastTrChangeEvent() {
    ...
    if ($tr.closest("tr").is(":last-child")) {
        //Get new empty row
        $.get('/SharedItemInvoices/GetRow', function (data) {
...
function submitEvent() {
    ...
    $trs.each(function (i, tr) {
        if (!$(tr).closest("tr").is(":last-child")) {
            ...
            $(tr).find('.js-store').attr('name', 'Items[' + i + '].StoreId');
        } else {
